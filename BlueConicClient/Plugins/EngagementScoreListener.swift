...
public class EngagementScoreListener: Plugin {
	...
	public override func onLoad() {
        ...
        if let context = self._context,
            ...
            let property = ListenerUtil.getProfilePropertyFromParameters(context.getParameters()) {
                let interests: [String]? = (ListenerUtil.getDictionaryFromParameters(self._context,key:"interests"))?["values"] as? [String]
                let decay = Int(ListenerUtil.getValueFromParameters(context, key: "decay"))

                BlueConicEventFactory.getInstance().cleanup()
                self._engagementService = EngagementService(client: self._client!, context: context, propertyName: property, decay: decay, isInterest: false, allInterests: interests)
